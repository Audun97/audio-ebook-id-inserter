...
## Possible abstraction of adding ids attributes
def add_ids(tag, id_counter, soup):
    # If there are not tags inside tag
        if isinstance(tag.string, NavigableString):
                ...
                if not re.search(r'[:;!,?.]', tag.string) and re.search(r'\w', tag.string):
                    current_span = soup.new_tag("span")
                    current_span["id"] = f"f{str(id_counter).zfill(3)}"
                    id_counter += 1
                    tag.wrap(current_span)
                    print("no comma \n")
                # if sentence contains comma, punctuation etc   
                elif re.search(r'\w', tag.string):
                    current_span = None
                    sentences = re.split(r'(?<=[.?!;:,])\s+(?=\w)', tag.string)
                    tag.clear()
                    ...
                    print("this was caught" + str(tag.string))
                return id_counter
